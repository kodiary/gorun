...
class DefaultController extends Controller {
	...
	public function actionLogin() {
		try {
			...
			Yii::app()->user->setFlash('hybridauth-error', "Something went wrong, did you cancel?".$e);
		...
		if ($identity->authenticate()) {
			...
			if (Yii::app()->user->isGuest) {
			  //die('2');
                ...
                $provider = $ha->getAdapter($_GET['provider']);
				...
				if (isset($_POST['email'])) {
                    ...
                    $user->password_real = $_POST['password_signup'];
                    $user->password = sha1($_POST['password_signup']);
                    $user->fname = $user_profile->firstName;
                    $user->lname = $user_profile->lastName;
                    //$user->is_verified = 1;
                    $user->gender = ($user_profile->gender=='male')?"1":"0";
