...
function initiateUpload(index)
{
    ...
    'debug':true,
    'multiple':true,
	'action':'<?php echo $this->createUrl('articles/upload')?>',
    ...
    'onComplete':function(id, fileName, responseJSON){
        $('.uploadControl').text('Add Photos');
            ...
            {
                $('#uploadList').append('<li class="items" id="image_'+index+'"><img src="'+responseJSON.imageThumb+'"/><a href="javascript:void(0);" class="btn btn-danger" onclick="$(this).closest(\'li\').remove();">Remove</a><textarea placeholder="Caption" name="ArticleFile['+index+'][title]"></textarea><input type="hidden" name="ArticleFile['+index+'][filename]" value="'+responseJSON.filename+'"/><div class="clear"></div></li>');
                index++;
            }
            else
            {
