...
class NewsController extends Controller
{
	...
	public function actionCreate()
	{
        ...
        Yii::app()->user->setFlash('info', '<strong>Please Note - </strong>Your news requires approval before being posted live.');
	...
	public function actionUpdate()
	{
        ...
        if($slug){
    		...
    		$model = Articles::model()->get_article_by_slug($slug);
    ...
    public function sendApprovalMail($id)
    {
        if($id){
            ...
            if($model){
                ...
                $company = Company::companyInfo($model->company_id);
                $com_name = $company->name;
                $com_email = $company->email;
                $replyTo = $com_email;
                $subject = "News approval request from $com_name";
                $body = $this->renderPartial('application.views.email.newsApproval',array('companyId'=>$model->company_id, 'com_name'=>$com_name, 'news_title'=>$model->title, 'news_id'=>$model->id, 'news_slug'=>$model->slug),true);
                $admin_emails = array();
                if($emails = AdminEmail::getAdminEmails()){
                    if($emails->email1!='') $admin_emails[]=$emails->email1;
                    if($emails->email2!='') $admin_emails[]=$emails->email2;
                    if($emails->email3!='') $admin_emails[]=$emails->email3;
                    if($emails->email4!='') $admin_emails[]=$emails->email4;
                    if($emails->email5!='') $admin_emails[]=$emails->email5;
                    $receiver_email = implode(',', $admin_emails);          
                }
                else $receiver_email = Yii::app()->params['adminEmail'];
                CommonClass::sendEmail("","", $receiver_email, $subject, $body,$replyTo);
            }
        }
    }
